...
for idx, (start_time, end_time) in enumerate(windows):
    ...
    for i, (audio_data, title) in enumerate(zip([audio_data_good, audio_data_faulty], 
        ...
        for j, (y, sr, S_db, mfcc) in enumerate(audio_data):
            ...
            S_db_segment = S_db[:, start_sample:end_sample] if end_sample > start_sample else np.zeros((S_db.shape[0], 1))

            # Extract the segment for MFCC
            mfcc_segment = mfcc[:, start_sample:end_sample] if end_sample > start_sample else np.zeros((mfcc.shape[0], 1))
